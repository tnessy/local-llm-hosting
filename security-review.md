...
> `15-workspaces.md §4c/§4d` predate the rewrite; the equivalent resources now
> live in `assets/k8s/` and are applied in step 04 (core) / step 14 (Authentik).

---
## CRITICAL

### ~~C-1~~ — ✅ FIXED (R1 + R2) — Calico CNI Not Pre-Validated — All NetworkPolicies Silently Unenforced if Missing
**Location:** 15-workspaces.md §1, §5  
**Flagged by:** NET-2  
...
**Location:** 06-gateway-litellm.md §2–3; 09-connectivity-cloudflare.md §4  
...
**Location:** 15-workspaces.md §5 RBAC — assets/k8s/llm-platform/orchestrator-rbac.yaml  
...
**Location:** 04-deploy-stack-ubuntu.md §2; 09-connectivity-cloudflare.md §5  
...
**R2 Fix:** Added explicit `sed -i` .env cleanup step in step 09 §5 post-migration; added docker inspect exposure warning in step 04 §2; added dqlite encryption note referencing EncryptionConfiguration (cross-ref H-14); expanded connector notifications to numbered substeps with dashboard verification.

---
## HIGH

### H-1 — api.domain.com Has No Cloudflare Access Authentication — Edge Is Fully Bypassed — **ACCEPTED RESIDUAL**
**Location:** 09-connectivity-cloudflare.md §4; assets/cloudflare-access-notes.md §3  
...
**Location:** 15-workspaces.md §4b — Default-deny baseline; §4c — llm-core policies; §4d — llm-platform policies  
**Flagged by:** NET-1  
...
**Location:** 15-workspaces.md §5 — Workspace launch sequence  
**Flagged by:** NET-3  
...
**Location:** 15-workspaces.md §3 (gateway.yaml)  
...
**Location:** 15-workspaces.md §6 — Workspace pod spec (volumes.tmp emptyDir: {})  
...
**Location:** 13-operations.md §Key & access hygiene; assets/.env.example  
...
**Location:** 15-workspaces.md §1 (MicroK8s setup)  
...
**Location:** 08-connectivity-tailscale.md §4; 02-host-os-ubuntu.md; assets/tailscale-acl.json lines 22–27  
...
**Fix (implemented):** Added mandatory UFW section (step 02 §4): `default deny incoming / default allow outgoing`, SSH allowed from operator-specified `<LAN_CIDR>`, monitoring placeholder for Prometheus scrape rules (Grafana host IP TBD — step 13). `ufw allow in on tailscale0` added to step 08 §1 post-`tailscale up`. Docker bypass mitigated via `"ip": "127.0.0.1"` merged into daemon.json in step 02 §7 (after nvidia-ctk writes the nvidia runtime entry) — loopback is now the default bind address for any future `ports:` entry that omits an explicit host IP; existing compose services already use `127.0.0.1:` bindings and `expose:` only. UFW manages ip6tables alongside iptables, so IPv6 internet exposure is also covered.

### ~~H-18~~ — ✅ FIXED — No Automatic Security Patching Configured on a 24/7 Internet-Facing Server
**Location:** 02-host-os-ubuntu.md §2; 13-operations.md  
**Flagged by:** HOST-3  
...
**Fix (addressed via H-17):** UFW `default deny incoming` (step 02 §4) blocks all inbound LAN access to port 16443; `ufw allow in on tailscale0` (step 08 §1) permits Tailscale-based remote kubectl. The apiserver bind-address is intentionally left at `0.0.0.0` — `--bind-address` takes a single IP, so binding to the Tailscale IP would break local `microk8s kubectl` which uses `127.0.0.1:16443` over loopback. Accepted residual: the apiserver process listens on all interfaces; UFW is the enforcement layer. Recovery path if Tailscale is unavailable: LAN SSH (port 22 open from LAN CIDR) → `microk8s kubectl` locally over loopback is always accessible from the host. Remote kubeconfig (Tailscale IP substitution) and verification steps added to step 15 §1.
...
**Location:** 15-workspaces.md §5 — Orchestrator namespace creation logic  
...
**Location:** 15-workspaces.md §5 — Idle TTL polling  
...
**Location:** 15-workspaces.md §5 — Destroy flow  
...
**Location:** 15-workspaces.md §5 and §6 — PVC persistence; 14-identity-sso.md — OIDC login flow  
...
**Location:** README.md (service map); assets/docker-compose.yml (cloudflared service); 15-workspaces.md §9  
...
**Location:** 13-operations.md — Updates section  
**Flagged by:** OPS-3  
...
**Location:** 13-operations.md — Monitoring section  
**Flagged by:** OPS-5  
...
**Fix (implemented):** Full off-server monitoring stack in 13-operations.md §Monitoring. The Grafana host is a separate machine on the same LAN (`<grafana-host-lan-ip>`):
...
**Location:** 15-workspaces.md §4a — workspace-isolation NetworkPolicy, kube-dns egress rule  
...
**Location:** 15-workspaces.md §4a — workspace-isolation NetworkPolicy, egress ipBlock except list  
...
**Location:** 09-connectivity-cloudflare.md §4; assets/cloudflare-access-notes.md §3–4  
...
**Location:** 15-workspaces.md §5 — Orchestrator; no NetworkPolicy defined for orchestrator  
**Flagged by:** NET-13  
...
**Location:** 15-workspaces.md §4a — workspace-isolation NetworkPolicy, ingress rule  
**Flagged by:** NET-14  
...
**Location:** 04-deploy-stack-ubuntu.md; 15-workspaces.md §1  
**Flagged by:** NET-15  
...
**Location:** 14-identity-sso.md §Setup outline step 1; 15-workspaces.md §Architecture  
...
**Location:** 14-identity-sso.md; 06-gateway-litellm.md  
**Flagged by:** AUTH-8  
...
**Fix (addressed by design):** The Admin UI (step 16) makes user deprovision a single operation that atomically revokes the LiteLLM key, removes the Authentik group membership, and deletes the Open WebUI account. No separate manual step required. The deprovision flow is documented in 16-admin-ui.md.

### M-12 — Open WebUI OIDC SSO Is Optional — Authentik Removal Does Not Disable WebUI Account
**Location:** 14-identity-sso.md §Setup outline step 5; 07-webui-open-webui.md; 09-connectivity-cloudflare.md §3  
...
**Location:** assets/workspace-base/Dockerfile CMD; 15-workspaces.md §9  
...
**Location:** 09-connectivity-cloudflare.md §3; assets/cloudflare-access-notes.md §2  
**Flagged by:** AUTH-14  
...
**Location:** 15-workspaces.md §9; 09-connectivity-cloudflare.md §2  
...
**Location:** 08-connectivity-tailscale.md §4 — Lock SSH to Tailscale  
**Flagged by:** NET-19  
...
**Location:** 15-workspaces.md §8; assets/litellm-config.yaml; assets/cloudflare-access-notes.md §1  
...
**Location:** 15-workspaces.md §5 — Orchestrator namespace creation; §6 Workspace pod spec  
...
**Location:** 15-workspaces.md §5 — Orchestrator RBAC (ClusterRole orchestrator)  
...
**Location:** 15-workspaces.md §3 — Traefik Gateway setup (gateway.yaml: allowedRoutes.namespaces.from: All)  
...
**Location:** 15-workspaces.md §6 — Workspace pod spec (containers[0].securityContext)  
...
**Location:** 15-workspaces.md §5 — Orchestrator RBAC (verbs: delete on namespaces, deployments)  
...
**Location:** 15-workspaces.md §7 — Resource management (workspace-quota ResourceQuota)  
...
**Fix:** Apply `chmod 600 /opt/home-llm/.env` immediately after creation (document this in step 04, not step 13), separate high-impact secrets (CF_TUNNEL_TOKEN, LITELLM_MASTER_KEY) into Docker secrets or a systemd credential store, and ensure backups exclude or encrypt the .env file.

### M-34 — etcd / dqlite Data at Rest Is Unencrypted — Kubernetes Secrets Readable from Host Filesystem
**Location:** 15-workspaces.md §5; assets/litellm-config.yaml; assets/docker-compose.yml  
...
**Location:** assets/tailscale-acl.json; 08-connectivity-tailscale.md §3  
**Flagged by:** HOST-11  
...
**Location:** 15-workspaces.md §1; 02-host-os-ubuntu.md  
**Flagged by:** HOST-13  
...
**Location:** 15-workspaces.md §4a (workspace-isolation NetworkPolicy); 13-operations.md Monitoring  
...
**Location:** 08-connectivity-tailscale.md §1  
...
**Location:** 13-operations.md §Backups  
...
**Fix:** Add rotation procedures for all three keys to step 13 (LITELLM_SALT_KEY rotation requires coordinated key reissuance to all users), schedule quarterly rotation for the master key, and document CF_TUNNEL_TOKEN rotation via the Cloudflare dashboard. Friend virtual key revocation/reissuance is handled through the Admin UI (step 16) as a routine operation.

### M-47 — LiteLLM Verbose Logging May Write API Keys to Docker Log Files Readable by docker Group
**Location:** 13-operations.md Monitoring; assets/litellm-config.yaml line 23  
...
**Location:** 14-identity-sso.md Setup outline step 1  
**Flagged by:** OPS-9  
...
**Issue:** WEBUI_SECRET_KEY rotation invalidates all active Open WebUI sessions simultaneously, but this is undocumented in step 13's rotation procedures, and the .env.example does not prevent the key from being left empty (which may cause a predictable default).  
...
**Fix:** Document in step 13 that WEBUI_SECRET_KEY rotation invalidates all sessions (requires a maintenance window), enforce a non-empty value in the .env.example with a startup health check, and add this key to the quarterly rotation schedule.

---
## MEDIUM

### M-52 — No Maximum Concurrent Workspace Limit — grp-workspaces Users Can Exhaust Host Resources
**Location:** 15-workspaces.md §5, §7  
...
**Location:** 15-workspaces.md §5; 14-identity-sso.md  
...
**Location:** 15-workspaces.md §4a, §6  
...
**Location:** 15-workspaces.md §3 (Traefik Helm install)  
...
**Location:** 09-connectivity-cloudflare.md §3; 14-identity-sso.md §3  
...
**Location:** 06-gateway-litellm.md §1; 09-connectivity-cloudflare.md §4  
...
**Location:** 13-operations.md; 15-workspaces.md  
...
**Fix:** Add the .env file to the backup procedure, encrypted separately (e.g. GPG-encrypted, stored in a password manager), and document explicitly in step 13 that LITELLM_SALT_KEY and litellm.db are inseparable for recovery.
...
**Location:** 15-workspaces.md §5 (orchestrator), §6 (workspace pod spec)  
...
**Location:** assets/litellm-config.yaml line 27; assets/docker-compose.yml; 13-operations.md Backups  
